...
var maxSubarraySumCircular = function(A) {
    let original = kandane(A);
    for(let i = 0; i < A.length; i ++){
        A[i] = -A[i];
    }
    let negativeOriginal = kandane(A);
    let sum = 0;
    for(let i = 0; i < A.length; i++){
        A[i] = -A[i];
        sum += A[i];
    }
    if(sum + negativeOriginal !== 0 ){
        return Math.max(original, sum + negativeOriginal)
    }else{
        return original;
    }
};

var kandane = function(A){
    let max_so_far = A[0],localMaximum = A[0];
    for(let i = 1; i < A.length; i++){
        localMaximum = Math.max(A[i], A[i] + localMaximum);
        max_so_far = Math.max(localMaximum, max_so_far);
    }
    return max_so_far;
}